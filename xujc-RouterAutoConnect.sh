...
check() {
  ...
  if [ $? -ne 0 ]; then
    echo "Campus Nwetwork Watch dog error: can't connect to: $url"
    exit 1
  fi
  ...
  if [ "$count" -eq 0 ]; then
    echo "Campus Network Watchdog nothing to do, the network maybe connected"
    return 1
  else
    echo "Campus Network Watchdog error: more than one redirect url"
    exit 1
  fi
}
...
logout() {
  ...
  if [ $? -ne 0 ]; then
    echo "Campus Network Watchdog error: logout failed"
  fi
}
...
if [ "$os" = "Darwin" ]; then
  ...
  echo "Campus Network watchdog error: unsupported platform"
  exit 1
fi
